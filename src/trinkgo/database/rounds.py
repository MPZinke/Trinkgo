...
@connect
def update_round_ended(cursor: psycopg2.extras.RealDictCursor, round_id: int) -> None:
	query = """
		UPDATE "Rounds"
		SET "ended" = TRUE
		WHERE "id" = %s;
...
@connect
def update_round_start(cursor: psycopg2.extras.RealDictCursor, round: Round) -> None:
	query = """
		UPDATE "Rounds"
	...
	round.start = cursor.fetchone()["start"]
